...
class TC_customize {
  ...
  function __construct () {
    ...
    add_action ( 'customize_register'				
                 , array( $this , 'tc_add_controls_class' ) ,10,1);

    //add the customizer built with the builder below
    add_action ( 'customize_register'				
                 , array( $this , 'tc_customize_register' ) ,20,1 );

    //ARGS : customizer section, settings and controls accepted arguments
    add_filter ( '__customize_args'					
                 , array( $this , 'tc_customize_arguments' ));

    //MAP  of the customizer : sections, settings, controls.
    add_filter ( '__customize_map'					
                 , array( $this , 'tc_customizer_map' ));

    //BUILDER of the customizer using args + map
    add_filter ( '__customize_factory'				
                 , array( $this , 'tc_customize_factory' ), 10, 3);

    //sanitazition filters
    add_filter ( '__sanitize_textarea'				
                 , array( $this , 'tc_sanitize_textarea' ));
    add_filter ( '__sanitize_number'				
                 , array( $this , 'tc_sanitize_number' ));
    add_filter ( '__sanitize_url'					
                 , array( $this , 'tc_sanitize_url' ));

    //preview script
    add_action ( 'customize_preview_init'			
                 , array( $this , 'tc_customize_preview_js' ));

    //add option to menus
    add_action ( 'admin_menu'						
                 , array( $this , 'tc_add_options_menu' ));
    add_action ( 'wp_before_admin_bar_render'		
                 , array( $this , 'tc_add_admin_bar_options_menu' ));

    //skin choices filter
    add_filter ( '__skin_choices'					
                 , array( $this , 'tc_skin_choices' ), 20);
